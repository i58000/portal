...
import { TOKEN_SECRET, TOKEN_COOKIE_KEY, TOKEN_COOKIE_MAXAGE } from '@config/index'
import {
    checkAccount,
    newAccount,
    queryAccount
...
router.post('/register', async (ctx, next) => {
    ...
    const { errCode, errMsg, token, account }: any = await newAccount({ username, password, confirm, email });
    if (!errCode) {
...
router.post('/login', async (ctx, next) => {
    ...
    const { errCode, errMsg, token, account }: any = await checkAccount(username, password);
    if (!errCode) {
        const { id, nickname } = account;
        ctx.body = {
            token,
